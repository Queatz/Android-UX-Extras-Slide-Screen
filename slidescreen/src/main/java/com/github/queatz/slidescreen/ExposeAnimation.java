...
class ExposeAnimation extends Handler {
    ...
    ExposeAnimation(SlideScreen slideScreen, boolean to) {
        this.slideScreen = slideScreen;
        from = this.slideScreen.currentScale;
        this.to = to ? .75f : 1;
        duration = 150;
        startOffsetTime = 0;
        alive = false;
    ...
    private void apply(float time) {
        slideScreen.setScale(to * time + from * (1.0f - time));
    }
}
